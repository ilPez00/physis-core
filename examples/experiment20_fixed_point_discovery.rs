...
fn build_and_print_graph(
    ...
    for a in 0..fps.len() {
        for b in (a + 1)..fps.len() {
            ...
            if bridge > 0 || sim > 0.5 {
                println!(
                    "  '{}' <-> '{}'  direct-sim={:.3}  bridge-count={}",
                    names[fps[a]], names[fps[b]], sim, bridge
                );
            }
        }
    }
...
fn main() {
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        report_fixed_points(
            "Dataset A, even-indexed subsample (MiniLM)",
            &even_names,
            &even_emb,
            2,
        );
        report_fixed_points(
            "Dataset A, odd-indexed subsample (MiniLM)",
            &odd_names,
            &odd_emb,
            2,
        );
        ...
        let real_fps_minilm =
            report_fixed_points("Real ontology (MiniLM, M=8)", &real_names, &real_minilm, 8);
        if bge_available {
            let bge = bge.as_ref().unwrap();
            let real_bge: Vec<Vec<f32>> = texts.iter().map(|t| bge.embed(t)).collect();
            let real_fps_bge =
                report_fixed_points("Real ontology (BGE, M=8)", &real_names, &real_bge, 8);
            ...
            let top_k_minilm: std::collections::HashSet<usize> =
                minilm_ranked.iter().take(k).copied().collect();
            let top_k_bge: std::collections::HashSet<usize> =
                bge_ranked.iter().take(k).copied().collect();
