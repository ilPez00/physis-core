//! EXPERIMENT 63: MEDIAN ESTIMATOR AT N=1e7 — precision check on E62.
//!
//! E62 (N=1e6) AGREEMENT: the median of R(k)/S_unit(k) recovered 2C2
//! (1.395/1.451 vs 1.32032±0.15) where the unweighted mean missed
//! (1.470/1.487). E63 asks whether the agreement holds at N=1e7 — the
//! E62 CI [1.279, 1.660] comfortably covers 1.32032, so this is a
//! precision check, not a direction check. Same estimator, same gates,
//! only n_max changes.
//! Frozen spec: research/impossible_machine/EXPERIMENT63_SPEC.md.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== EXPERIMENT 63: MEDIAN ESTIMATOR AT N=1e7 ===");
    ...
    // E62 baseline: median estimator at N=1e6.
    let (e62d, e62h) = (1.3954335335712698f64, 1.4511018327787424f64);
    // E63: same estimator at N=1e7.
    ...
    println!("median fit disc={:.3} [{:.3},{:.3}] hold={:.3} [{:.3},{:.3}] tgt={:.5} (E62 N=1e6 {:.3}/{:.3})",
    ...
    let delta = dt_e62 - dt; // +ve = N=1e7 closer to 1.32032 than N=1e6
    let verdict = if sv {
        format!("AGREEMENT — 2C2 disc={:.3} hold={:.3} in {:.5}±{:.2} at N=1e7 with the median estimator (E62 N=1e6 {:.3}/{:.3}, Δ {:+.3}); G1 D={:.3} vs nulls {:.3}; G2 stable. Agreement holds at higher resolution.",
            fd, fh, C2X2, G3H, e62d, e62h, delta, dd, mx)
    } else {
        let mut r = Vec::new();
        if !g1 { r.push("G1 fail".to_string()); }
        if !g2 { r.push("G2 fail".to_string()); }
        if !g3 { r.push(format!("G3 miss: {:.3}/{:.3} vs {:.5}±{:.2} (E62 N=1e6 {:.3}/{:.3}, Δ {:+.3})",
            fd, fh, C2X2, G3H, e62d, e62h, delta)); }
        if delta > 0.02 {
            format!("PARTIAL — {}. Closer at N=1e7; needs N=1e8.", r.join("; "))
        } else if delta > -0.02 {
            format!("PARTIAL — {}. Flat between N=1e6 and N=1e7; estimator stable but imprecise.", r.join("; "))
        } else {
            format!("MISS — {}. Diverged from E62 with N; agreement was a low-N artifact.", r.join("; "))
        }
    };
    let rel = format!("no 1/2 in gen/score; median fit {:.3}/{:.3} at N=1e7 (E62 {:.3}/{:.3}); Δ {:+.3}; D {:.3}/{:.3}",
        fd, fh, e62d, e62h, delta, dd, dh);
    let out = E63Output {
        title: "Median-of-R(k)/S_unit(k) singular-series fit (2C2) — N=1e7 precision check".into(),
        context: "E62 (N=1e6) AGREEMENT with the median estimator (1.395/1.451 vs 1.32032±0.15). E63 runs the same estimator at N=1e7 to confirm the agreement holds at higher resolution; the E62 CI [1.279, 1.660] comfortably covers 1.32032, so this is a precision check, not a direction check.".to_string(),
        ...
        artifacts: vec![
            "physis-core/examples/experiment63_median_n1e7.rs".into(),
            "physis-core/research/impossible_machine/experiments/experiment63_results.json".into(),
            "physis-core/research/impossible_machine/EXPERIMENT63_RESULTS.md".into(),
        ],
    };
    let op = Path::new("research/impossible_machine/experiments/experiment63_results.json");
