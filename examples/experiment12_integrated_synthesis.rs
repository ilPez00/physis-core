...
fn normalized_mutual_info(a: &[usize], b: &[usize]) -> f64 {
    ...
    let h_b: f64 = col
        ...
        .sum();
    if h_a <= 1e-12 || h_b <= 1e-12 {
        return if mi.abs() < 1e-9 { 1.0 } else { 0.0 };
    }
...
fn run_pipeline<F: Fn(&str) -> Vec<f32>>(representation: &'static str, embed: F) -> PipelineResult {
    ...
    if std::env::var("DEBUG_SIMS").is_ok() {
        for i in [0usize, 5, 10] {
            ...
            println!(
                "DEBUG [{representation}] PURE  {:<20} true={:<12} sims={v:?}",
                PURE[i].0, PURE[i].4
            );
        }
        for i in 0..cross_sims.len() {
            ...
            v.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
            println!(
                "DEBUG [{representation}] CROSS {:<20} true=({},{}) sims={v:?}",
                CROSS[i].0, CROSS[i].3, CROSS[i].4
            );
        }
    }
    ...
    for (i, s) in pure_sims.iter().enumerate() {
        if argmax_cat(s) == fine_true[i] {
            a_hits += 1;
        }
    }
    for (i, s) in cross_sims.iter().enumerate() {
        ...
        if top == f1 || top == f2 {
            a_hits += 1;
        } // best-case credit, Iteration 8 style
    }
    ...
    let sweep: Vec<(f32, f32, usize)> = (0..=30)
        .map(|i| {
            let delta = i as f32 * 0.01;
            let fp = pure_sims
                .iter()
                .filter(|s| membership_set(s, delta).len() > 1)
                .count();
            let fpr = fp as f32 / n_pure as f32;
            let recall = cross_sims
                .iter()
                .enumerate()
                .filter(|(i, s)| {
    ...
    if std::env::var("DEBUG_SIMS").is_ok() {
        ...
        for (d, fpr, recall) in &sweep {
            println!("  {d:.2}  fpr={fpr:.3}  recall={recall}/{n_cross}");
        }
    }
    let max_recall = sweep.iter().map(|(_, _, r)| *r).max().unwrap();
    let (calibration_delta, calibration_fpr, calibration_recall) = sweep
        .iter()
    ...
    for coarse in ["mechanical", "driver", "economic"] {
        ...
        let pure_local_assignment: Vec<usize> = pure_members
            .iter()
            .map(|&gi| {
        ...
        let branch_purity = if pure_members.is_empty() {
            0.0
        } else {
            purity_k(&pure_local_assignment, 2, &pure_local_labels)
        };
        let pure_true_ids = label_ids(&pure_local_labels);
        let ari = if pure_members.len() > 1 {
            adjusted_rand_index(&pure_local_assignment, &pure_true_ids)
        } else {
            1.0
        };
        let nmi = if pure_members.len() > 1 {
            normalized_mutual_info(&pure_local_assignment, &pure_true_ids)
        } else {
            1.0
        };
        ...
        for c in 0..2 {
            let leaf_members: Vec<usize> = members
                .iter()
                .enumerate()
                .filter(|(li, _)| split[*li] == c)
                .map(|(_, &gi)| gi)
                .collect();
            if leaf_members.is_empty() {
                continue;
            }
    ...
    for (li, leaf) in leaves.iter().enumerate() {
        for &gi in &leaf.members {
            let sims = if gi < n_pure {
                &pure_sims[gi]
            } else {
                &cross_sims[gi - n_pure]
            };
            ...
            for &cat in &ms {
                ...
                if let Some(target) = leaves
                    .iter()
                    .position(|l| l.label.ends_with(&format!(".{cat}")))
                {
                    if target == li {
                        continue;
                    }
                    let name = if gi < n_pure {
                        PURE[gi].0
                    } else {
                        CROSS[gi - n_pure].0
                    };
    ...
    for leaf in &leaves {
        adjacency
            .entry(leaf.coarse.to_string())
            .or_default()
            .push(leaf.label.clone());
        adjacency
            .entry(leaf.label.clone())
            .or_default()
            .push(leaf.coarse.to_string());
    }
    ...
    let (dag_naive_calls, dag_max_depth_reached, dag_guarded_calls) =
        if let Some((_, start, _)) = cross_links.first() {
...
fn print_report(r: &PipelineResult) {
    ...
    println!(
        "Baseline A (raw cosine argmax, best-case credit for cross items): purity = {:.3}",
        r.baseline_a_purity
    );
...
fn main() {
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        println!(
            "{:<45} {:>10.3} {:>12.3}",
            "Baseline A purity", bare_result.baseline_a_purity, ctx_result.baseline_a_purity
        );
        println!(
            "{:<45} {:>10.3} {:>12.3}",
            "Baseline B (flat k-means) purity",
            bare_result.flat_kmeans_purity,
            ctx_result.flat_kmeans_purity
        );
        println!(
            "{:<45} {:>10.3} {:>12.3}",
            "Baseline B ARI", bare_result.flat_kmeans_ari, ctx_result.flat_kmeans_ari
        );
        println!(
            "{:<45} {:>10.3} {:>12.3}",
            "Baseline B NMI", bare_result.flat_kmeans_nmi, ctx_result.flat_kmeans_nmi
        );
        println!(
            "{:<45} {:>10.2} {:>12.2}",
            "Baseline C calibrated delta",
            bare_result.calibration_delta,
            ctx_result.calibration_delta
        );
        println!(
            "{:<45} {:>10} {:>12}",
            "Baseline C recall (of 5 cross items)",
            bare_result.calibration_recall,
            ctx_result.calibration_recall
        );
        println!(
            "{:<45} {:>10} {:>12}",
            "cross-links discovered",
            bare_result.cross_links.len(),
            ctx_result.cross_links.len()
        );
        ...
        println!(
            "{:<45} {:>10?} {:>12?}",
            "per-branch gate decisions", gates_bare, gates_ctx
        );
