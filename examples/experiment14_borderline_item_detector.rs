...
fn worst_subcluster_signal(embeddings: &[Vec<f32>], assignment: &[usize]) -> (f32, bool) {
    ...
    for parent in 0..2 {
        let members: Vec<usize> = (0..embeddings.len())
            .filter(|&i| assignment[i] == parent)
            .collect();
        if members.len() < 4 {
            continue;
        } // too small to meaningfully sub-bisect
        ...
        let mean_pairwise = |group: usize| -> f32 {
            let idx: Vec<usize> = (0..sub_emb.len())
                .filter(|&i| sub_split[i] == group)
                .collect();
            if idx.len() < 2 {
                return 1.0;
            }
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        let cases = vec![
            ...
            GateCase {
                ...
                correct: false,
                embeddings: DATASET_A
                    .iter()
                    .map(|(_, s, _)| embedder.embed(s))
                    .collect(),
                assignment: a_real,
                item_names: DATASET_A.iter().map(|(n, ..)| *n).collect(),
            },
            GateCase {
            ...
            GateCase {
                ...
                item_names: mech_names,
            },
        ];

        println!(
            "{:<48} {:>8} {:>10} {:>10} {:>10} {:>12} {:>8}",
            "case", "balance", "misfit%", "worst-marg", "margin-std", "sub-bal", "tighter"
        );
        ...
        println!(
            "misfit fraction (inverted, higher=better):        {}",
            separates(&misfit_scores)
        );
        println!(
            "worst-item margin (higher=better, less negative): {}",
            separates(&worst_scores)
        );
        println!(
            "margin std-dev, LOWER=better hypothesis:          {}",
            separates(&std_scores)
        );
        let std_scores_higher_better: Vec<(bool, f32)> =
            std_scores.iter().map(|(c, s)| (*c, -s)).collect();
        println!(
            "margin std-dev, HIGHER=better hypothesis:         {}",
            separates(&std_scores_higher_better)
        );
        println!(
            "sub-bisection contamination signal (genuinely different mechanism): {}",
            separates(&subcluster_scores)
        );
