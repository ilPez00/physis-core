...
const DOMAINS_TRAIN: &[DomVocab] = &[
    DomVocab {
        ...
        settings: &[
            "by the loading bay",
            "under the steel gantry",
            "beside the tool wall",
            "near the furnace",
    ...
    DomVocab {
        ...
        objects: &[
            ...
            "a lawnmower",
            "a flowerbed border",
            "a basket of cuttings",
        ],
        settings: &[
            "under the pear trees",
            "by the greenhouse wall",
...
const DOMAINS_TRANSFER: &[DomVocab] = &[
    ...
    DomVocab {
        ...
        objects: &[
            ...
            "a jar of glaze",
            "a litho stone",
            "a spool of warp thread",
            "a chisel set",
        ],
        settings: &[
            "by the north window",
            "under the mezzanine",
            "along the drying wall",
            "near the sink",
            "beside the flat files",
            "in the spray booth",
        ],
    },
    DomVocab {
    ...
    DomVocab {
        ...
        objects: &[
            "a HF receiver",
            "a coil of antenna wire",
            "a signal lamp",
            "a logbook",
            "a bank of switches",
            "a morse key",
            "a headset",
            "a crate of vacuum tubes",
            "a battery rack",
            "an oscilloscope",
            "a patch panel",
            "a dial gauge",
        ...
        settings: &[
            "by the antenna mast",
            "under the noise filter",
            "along the patch bay",
            "near the transformer",
            "beside the blackout curtain",
            "in the relay closet",
        ],
    },
];
...
fn gen_sentence(rng: &mut StdRng, dom: &DomVocab, flow: usize, p_cue: f64, q_cue: f64) -> String {
    ...
    let verb_pool: &[&str] = if rng.gen::<f64>() < q_cue {
        FLOW_VERB[flow]
    } else {
        NEUTRAL_VERB
    };
...
fn silhouette(data: &[Vec<f32>], part: &[usize]) -> f64 {
    ...
    for i in 0..n {
        clusters.entry(part[i]).or_default().push(i);
    }
    ...
    for &ci in &ids {
        ...
        for (pos, &i) in members.iter().enumerate() {
            ...
            for &cj in &ids {
                ...
                let s: f64 = other
                    .iter()
                    .map(|&j| (1.0 - cosine_sim(&data[i], &data[j])) as f64)
                    .sum();
...
fn kmeans(data: &[Vec<f32>], k: usize, rng: &mut StdRng, iters: usize) -> Vec<usize> {
    ...
    let mut dist = |x: &Vec<f32>, cs: &[Vec<f32>]| -> f64 {
        cs.iter()
            .map(|c| (1.0 - cosine_sim(x, c)) as f64)
    ...
    for _ in 0..iters {
        ...
        for c in 0..k {
            if counts[c] == 0 {
                ...
                let (far, _) = (0..n)
                    .map(|i| (i, (1.0 - cosine_sim(&data[i], &centers[assign[i]])) as f64))
...
fn cos_matrix(data: &[Vec<f32>]) -> Vec<Vec<f32>> {
    let n = data.len();
    (0..n)
        .map(|i| (0..n).map(|j| cosine_sim(&data[i], &data[j])).collect())
        .collect()
}
...
fn extract_residual(m: &[Vec<f32>], baseline: &[usize], k: usize, rng: &mut StdRng) -> Residual {
    ...
    let means_r = &means;
    let cross_w: Vec<(usize, usize, f32)> = (0..n)
        .flat_map(|i| {
            let bi = baseline[i];
            let row = &m[i];
...
fn discover_under(
    ...
    for i in 0..train_emb.len() {
        ...
        if best_j != usize::MAX && partition[best_j] == partition[i] {
            hits += 1;
        }
    }
...
fn build_candidates(
    ...
    // P_edge — partition the residual graph itself; references = community centroids
    {
        ...
        let mut groups: HashMap<usize, Vec<usize>> = HashMap::new();
        for (i, &g) in communities.iter().enumerate() {
            groups.entry(g).or_default().push(i);
        }
        let mut keys: Vec<usize> = groups.keys().copied().collect();
        keys.sort_unstable();
        let vecs: Vec<Vec<f32>> = keys
            .iter()
            .map(|&g| {
    ...
    {
        let full: Vec<(usize, usize, f32)> = (0..n)
            .flat_map(|i| (i + 1..n).map(move |j| (i, j, m[i][j])))
            .collect();
    ...
    // P_ref-randref — ablation: same count of references, chosen at random
    {
        ...
        idx.shuffle(rng);
        let vecs: Vec<Vec<f32>> = idx
            .iter()
            .take(N_REF_REFS)
            .map(|&i| train_emb[i].clone())
            .collect();
        cands.push(Candidate {
...
fn heldout_eval(
    ...
    for &i in &idxs {
        ...
        if train_latent[bj] == test_latent[i] {
            correct += 1;
        }
    }
    ...
    for (a, &i) in idxs.iter().enumerate() {
        for &j in idxs[a + 1..].iter() {
            if test_surface[i] == test_surface[j] {
                continue;
            }
...
const PALETTE: [&str; 8] = [
    "#4269d0", "#efb118", "#ff725c", "#6cc5b0", "#3ca951", "#a463f2", "#ff8ab7", "#9498a0",
];
...
fn tree_svg(nodes: &[(String, Option<String>, f64)], title: &str) -> String {
    ...
    for (id, parent, gain) in nodes {
        ...
        if let Some(p) = parent {
            s.push_str(&format!(
                "<line x1=\"360\" y1=\"{:.0}\" x2=\"{}\" y2=\"{:.0}\" stroke=\"#889\"/>\n",
                y + 13.0,
                x,
                y + 13.0
            ));
...
fn run_pipeline(
    ...
    for k in 2..=6 {
        ...
        let better = match &baseline {
            None => true,
            Some((bs, _, _)) => s > *bs + 1e-9,
        };
        if better {
            baseline = Some((s, k, part));
        }
    }
    ...
    let mut groups: HashMap<usize, Vec<usize>> = HashMap::new();
    for (i, &g) in baseline_part.iter().enumerate() {
        groups.entry(g).or_default().push(i);
    }
    let mut coh = 0.0f64;
    let mut coh_n = 0usize;
    ...
    for i in 0..n {
        ...
        if baseline_part[bj] == baseline_part[i] {
            hits += 1;
        }
    }
    ...
    let top_items: Vec<String> = deg_order
        .iter()
        .take(N_REF_REFS)
        .map(|&i| train_texts[i].clone())
        .collect();
    let res_info = ResidualInfo {
        n_edges: residual.edges.len(),
        null_p95: residual.null_p95,
        top_residual_items: top_items.clone(),
        edges: residual.edges.clone(),
    };

    // baseline centroids for P_soft + oracle
    let mut base_groups: HashMap<usize, Vec<usize>> = HashMap::new();
    for (i, &g) in baseline_part.iter().enumerate() {
        base_groups.entry(g).or_default().push(i);
    }
    let mut base_keys: Vec<usize> = base_groups.keys().copied().collect();
    base_keys.sort_unstable();
    let baseline_centroids: Vec<Vec<f32>> = base_keys
        .iter()
        .map(|&g| {
    ...
    let mut cands = build_candidates(
        &residual,
        &m,
        train_emb,
        &train_texts,
    ...
    for (i, &f) in train_latent.iter().enumerate() {
        oracle_groups.entry(f).or_default().push(i);
    }
    let oracle_vecs: Vec<Vec<f32>> = (0..N_FLOWS)
        .map(|f| {
            let vs: Vec<&Vec<f32>> = oracle_groups[&f].iter().map(|&i| &train_emb[i]).collect();
            centroid_of(&vs)
        })
        .collect();
    let oracle_transformed: Vec<Vec<f32>> = train_emb
        .iter()
        .map(|x| transform_via_refs(x, &oracle_vecs))
        .collect();
    ...
    let winner_idx = perspectives
        .iter()
        .enumerate()
        .max_by(|a, b| {
            a.1.silhouette_raw_after
                .partial_cmp(&b.1.silhouette_raw_after)
                .unwrap()
        })
        .map(|(i, _)| i)
        .unwrap();
    ...
    let coh_pick = perspectives
        .iter()
        .enumerate()
        .max_by(|a, b| {
            a.1.coherence_after
                .partial_cmp(&b.1.coherence_after)
                .unwrap()
        })
        .map(|(i, _)| i)
        .unwrap();
    let rand_pick = rng.gen_range(0..perspectives.len());
    let winner_build = std::mem::replace(
        &mut cands[winner_idx].build,
        Box::new(|x: &[Vec<f32>]| x.to_vec()),
    );
    let coh_build = std::mem::replace(
        &mut cands[coh_pick].build,
        Box::new(|x: &[Vec<f32>]| x.to_vec()),
    );
    let rand_build = std::mem::replace(
        &mut cands[rand_pick].build,
        Box::new(|x: &[Vec<f32>]| x.to_vec()),
    );
    let selection_controls = vec![
        SelectionControl {
            rule: "max-raw-silhouette".into(),
            selected: winner_id.clone(),
        ...
        SelectionControl {
            rule: "max-own-space-coherence".into(),
            selected: perspectives[coh_pick].id.clone(),
            heldout: heldout_eval(
                &perspectives[coh_pick].id,
                train_emb,
                test_emb,
                coh_build.as_ref(),
                &train_latent,
        ...
        SelectionControl {
            rule: "random".into(),
            selected: perspectives[rand_pick].id.clone(),
            heldout: heldout_eval(
                &perspectives[rand_pick].id,
                train_emb,
                test_emb,
                rand_build.as_ref(),
                &train_latent,
    ...
    let mut accepted = false;
    {
        ...
        for level in 2..=3 {
            ...
            for (i, &g) in parent_part.iter().enumerate() {
                c2groups.entry(g).or_default().push(i);
            }
            let mut c2keys: Vec<usize> = c2groups.keys().copied().collect();
            c2keys.sort_unstable();
            let parent_centroids: Vec<Vec<f32>> = c2keys
                .iter()
                .map(|&g| {
            ...
            )> = Vec::new();
            {
                ...
                for (i, &g) in comm.iter().enumerate() {
                    grp.entry(g).or_default().push(i);
                }
                ...
                children.push((
                    ...
                    Vec::new(),
                    vecs.clone(),
                    "cosine-to-references",
                    train_emb
                        .iter()
                        .map(|x| transform_via_refs(x, &vecs))
                        .collect(),
                ));
            }
            {
            ...
            for (name, texts, vecs, op, transformed) in children {
                ...
                if better {
                    best_child = Some((p.silhouette_raw_after, p));
                }
            }
            match best_child {
    ...
    for a in 0..route_names.len() {
        for b in (a + 1)..route_names.len() {
            let pa = &perspectives
                .iter()
                .find(|p| p.id == route_names[a])
                .unwrap()
                .partition;
            let pb = &perspectives
                .iter()
                .find(|p| p.id == route_names[b])
                .unwrap()
                .partition;
            route_pairs.push(RoutePair {
                a: route_names[a].into(),
                b: route_names[b].into(),
                ari: ari(pa, pb),
                nmi: nmi(pa, pb),
            });
        }
    ...
    let psoft = &perspectives
        .iter()
        .find(|p| p.id == "P_soft")
        .unwrap()
        .partition;
    let pref = &perspectives
        .iter()
        .find(|p| p.id == "P_ref")
        .unwrap()
        .partition;
    ...
    let convergence = ConvergenceReport {
        ...
        consensus_pair_precision: if n_cons > 0 {
            n_cons_lat as f64 / n_cons as f64
        } else {
            0.0
        },
        consensus_pair_recall: if n_lat_total > 0 {
            n_cons_lat as f64 / n_lat_total as f64
        } else {
            0.0
        },
    ...
    if accepted {
        ...
        let child_build = move |items: &[Vec<f32>]| -> Vec<Vec<f32>> {
            items
                .iter()
                .map(|x| transform_via_refs(x, &child_vecs))
                .collect()
        };
        heldout.push(heldout_eval(
            "winner-child",
    ...
    let oracle_heldout = heldout_eval(
        "P_oracle",
        train_emb,
        test_emb,
        &oracle_build,
    ...
    transfer_reports.push(heldout_eval(
        "raw",
        train_emb,
        transfer_emb,
        &identity,
    ...
    for krefs in [3usize, 6, 12] {
        let vecs: Vec<Vec<f32>> = top_items
            .iter()
            .take(krefs)
            .zip(deg_order.iter().take(krefs))
            .map(|(_, &i)| train_emb[i].clone())
            .collect();
        let build = move |items: &[Vec<f32>]| -> Vec<Vec<f32>> {
            items.iter().map(|x| transform_via_refs(x, &vecs)).collect()
        };
        let rep = heldout_eval(
            "P_ref-topk",
            train_emb,
            test_emb,
            &build,
        ...
        compression.push(CompressionPoint {
            method: "P_ref-topk".into(),
            n_refs: krefs,
            acc_1nn: rep.acc_1nn,
        });
    }
    ...
    let stability_winner = {
        ...
        for _ in 0..5 {
            ...
            let w_restrict: Vec<usize> = sub
                .iter()
                .map(|&i| perspectives[winner_idx].partition[i])
                .collect();
...
fn resolve_tiers(out: &PipelineOutput, cross_pairs: &[RoutePair]) -> (u8, Vec<(String, String)>) {
    ...
    let w_transfer = heldout_of(&out.transfer, &out.winner_id)
        .unwrap_or_else(|| heldout_of(&out.transfer, "raw").unwrap());
    ...
    verdicts.push((
        "T1 train organization".into(),
        if tier1 {
            "PASS".into()
        } else {
            format!(
                "FAIL (winner ARI {:.3} vs base {:.3} / rand {:.3}, raw-sil {:.4} vs {:.4})",
                winner.latent_ari,
                out.baseline_latent_ari,
                rand_c.latent_ari,
                winner.silhouette_raw_after,
                out.baseline_silhouette
            )
        },
    ));
    ...
    let rand_control_ok =
        rand_held.acc_1nn < raw.acc_1nn + 0.05 && rand_held.acc_1nn < w_held.acc_1nn - 0.02;
    let t2 =
        tier1 && w_held.acc_1nn >= raw.acc_1nn + 0.05 && w_held.link_auc >= raw.link_auc + 0.05;
    verdicts.push((
        "T2 held-out".into(),
        if t2 {
            "PASS".into()
            ...
            format!(
                "FAIL (held acc {:+.3} vs raw {:.3}, AUC {:+.3} vs {:.3})",
                w_held.acc_1nn - raw.acc_1nn,
                raw.acc_1nn,
                w_held.link_auc - raw.link_auc,
                raw.link_auc
            )
        },
    ));
    ...
    let route_names = ["P_edge", "P_soft", "P_ref"];
    let routes_ok: Vec<&str> = route_names
        .iter()
        .copied()
        .filter(|rn| {
            heldout_of(&out.heldout, rn)
                .map(margin_routes_ok)
                .unwrap_or(false)
        })
        .collect();
    let max_route_ari = out
        .convergence
        .route_pairs
        .iter()
        .map(|p| p.ari)
    ...
    verdicts.push((
        ...
        if t4 {
            ...
            format!(
                "FAIL (transfer acc {:+.3} vs raw {:.3})",
                w_transfer.acc_1nn - raw_transfer.acc_1nn,
                raw_transfer.acc_1nn
            )
        },
    ));
    ...
    if out.recursion_accepted {
        ...
        let keeps = child_held
            .map(|c| c.acc_1nn >= w_held.acc_1nn - 0.02)
            .unwrap_or(false);
        verdicts.push((
            "T5 recursion keeps gain".into(),
            if keeps {
                "PASS".into()
            } else {
                "FAIL (child drops held-out gain)".into()
            },
        ));
    } else {
        verdicts.push((
            "T5 recursion keeps gain".into(),
            "FAIL (recursion stopped: no internal gain at level 2)".into(),
        ));
    }
...
fn calibration(e: &dyn VectorEmbed, seed: u64) -> (CalibPoint, Vec<CalibPoint>) {
    ...
    for p in ps {
        for q in qs {
            let mut rng = StdRng::seed_from_u64(
                seed.wrapping_add((p * 1000.0) as u64)
                    .wrapping_add((q * 77.0) as u64),
            );
            ...
            pts.push(CalibPoint {
                ...
                cross_flow_margin: if ns > 0 && nd > 0 {
    ...
    let mut eligible: Vec<&CalibPoint> = pts
        .iter()
        .filter(|c| c.surface_purity >= 0.7 && c.latent_ari >= 0.05 && c.cross_flow_margin >= 0.005)
        .collect();
    if eligible.is_empty() {
        eprintln!(
            "warning: no calibration point satisfied the difficulty window; relaxing (recorded)"
        );
...
fn run(args: &[String]) {
    ...
    let seed = parse_flag(args, "--seed")
        ...
        .unwrap_or(20260911);
    let out_dir =
        parse_flag(args, "--out").unwrap_or_else(|| "research/perspective_invention".into());
    ...
    if probe_only {
        write_file(
            &std::path::Path::new(&out_dir).join("datasets/calibration.json"),
            &serde_json::to_string_pretty(
                &serde_json::json!({ "seed": seed, "chosen": chosen, "grid": calib }),
            )
            .unwrap(),
        );
    ...
    println!(
        "dataset: train {} / test {} / transfer {} (transfer domains differ)",
        splits.train.len(),
        splits.test.len(),
        splits.transfer.len()
    );
    ...
    write_file(
        &std::path::Path::new(&out_dir).join("datasets/dataset_sidecar.json"),
    ...
    let all_texts: Vec<String> = splits
        .train
        .iter()
        .chain(splits.test.iter())
        .chain(splits.transfer.iter())
    ...
    write_file(
        &std::path::Path::new(&out_dir).join("experiments/results.json"),
...
fn print_summary(
    out: &PipelineOutput,
    out_b: &Option<PipelineOutput>,
    tier: u8,
    verdicts: &[(String, String)],
) {
    println!("\n════════════ SUMMARY ({}) ════════════", out.embedder);
    println!(
        "baseline: k={} surface purity {:.3} latent ARI {:.3} raw-sil {:.4}",
        out.baseline_k,
        out.baseline_surface_purity,
        out.baseline_latent_ari,
        out.baseline_silhouette
    );
    println!(
        "residual: {} cross-cluster edges (null p95 {:+.4})",
        out.residual.n_edges, out.residual.null_p95
    );
    println!("\ncandidates (internal selection = max raw-space silhouette):\n");
    println!(
        "  {:<16} {:<26} {:>10} {:>9} {:>9} {:>9} {:>9}",
        "id", "generator", "raw-sil", "own-sil", "ARI", "NMI", "purity"
    );
    for p in &out.candidates {
        println!(
            "  {:<16} {:<26} {:>10.4} {:>9.4} {:>9.4} {:>9.4} {:>9.4}{}",
            p.id,
            p.generator,
            p.silhouette_raw_after,
            p.silhouette_own_space,
            p.latent_ari,
            p.latent_nmi,
            p.latent_purity,
            if p.id == out.winner_id {
                "  <— winner"
            } else {
                ""
            }
    ...
    for rp in &out.convergence.route_pairs {
        println!(
            "  {} vs {}: ARI {:.3} NMI {:.3}",
            rp.a, rp.b, rp.ari, rp.nmi
        );
    }
    println!(
        "  permuted null ARI: mean {:.3} p95 {:.3}",
        out.convergence.permuted_null_ari_mean, out.convergence.permuted_null_ari_p95
    );
    println!(
        "  consensus pairs: {} precision {:.3} recall {:.3}",
        out.convergence.n_consensus_pairs,
        out.convergence.consensus_pair_precision,
        out.convergence.consensus_pair_recall
    );
    for xp in &out.convergence.cross_embedder {
        println!(
            "  cross-embedder {} vs {}: ARI {:.3} (known null ≈ 0.10)",
            xp.a, xp.b, xp.ari
        );
    }
    ...
    for h in &out.transfer {
        println!(
            "  {:<16} acc {:.3} CI [{:.3},{:.3}]  AUC {:.3}",
            h.method, h.acc_1nn, h.acc_1nn_ci.0, h.acc_1nn_ci.1, h.link_auc
        );
    }
    ...
    println!(
        "\nstability: winner {:.3} baseline {:.3}",
        out.stability_winner, out.stability_baseline
    );
    println!("selection controls:");
    for s in &out.selection_controls {
        println!(
            "  {} → {} held acc {:.3} AUC {:.3}",
            s.rule, s.selected, s.heldout.acc_1nn, s.heldout.link_auc
        );
    }
    println!(
        "\nrecursion: {}",
        if out.recursion_accepted {
            ...
            "stopped (no internal gain)"
        }
    );
    for p in &out.recursion {
        println!(
            "  {} (parent {:?}) raw-sil {:.4} ARI {:.3}",
            p.id, p.parent, p.silhouette_raw_after, p.latent_ari
        );
    }
    println!("\n════════════ TIER {} ════════════", tier);
    ...
    let tier_name = match tier {
        0 => "FAILED",
        1 => "INTERESTING BUT INSUFFICIENT",
        2 => "SUPPORTED",
        3 => "STRONG STRUCTURAL EVIDENCE",
        4 => "REPRESENTATION DISCOVERY",
        _ => "HIGH-ORDER GENERATIVE REPRESENTATION",
    };
    println!("  → {tier_name}");
}
...
fn emit_svgs(
    ...
    let winner = out
        ...
        .unwrap();
    write_file(
        &viz.join("train_by_surface.svg"),
        &scatter_svg(
            &pts,
            &train_surface,
            "train — colored by SURFACE domain (the dominant signal)",
            "PCA-2D of MiniLM embeddings; this is the organization ordinary classification finds",
        ),
    );
    write_file(
        &viz.join("train_by_latent.svg"),
        &scatter_svg(
            &pts,
            &train_latent,
            "train — colored by LATENT flow (the true organizing principle)",
            "evaluation-only coloring; discovery never saw these labels",
        ),
    );
    write_file(
        &viz.join("train_discovered.svg"),
        &scatter_svg(
            &pts,
            &winner.partition,
            &format!("train — discovered partition D({})", out.winner_id),
            &format!(
                "raw-space silhouette {:.4} vs baseline {:.4}",
                winner.silhouette_raw_after, out.baseline_silhouette
            ),
        ),
    );
    ...
    write_file(
        &viz.join("residual_graph.svg"),
        &residual_matrix_svg(
            &out.residual.edges,
            &order,
            &surface_blocks,
            "residual graph — cross-cluster relations the baseline partition discarded",
        ),
    );
    // agreement heatmap
    let mut labels: Vec<String> = Vec::new();
    let mut parts: Vec<Vec<usize>> = Vec::new();
    labels.push("P_edge".into());
    parts.push(
        out.candidates
            .iter()
            .find(|p| p.id == "P_edge")
    ...
    labels.push("winner".into());
    parts.push(winner.partition.clone());
    let mut vals: Vec<Vec<f64>> = Vec::new();
    for a in &parts {
        vals.push(parts.iter().map(|b| ari(a, b)).collect());
    }
    write_file(
        &viz.join("cross_perspective_agreement.svg"),
        &heatmap_svg(
            &labels,
            &vals,
            "partition agreement (ARI) across independent routes",
        ),
    );
    // lineage tree
    let mut tree: Vec<(String, Option<String>, f64)> = vec![
        ("baseline".into(), None, 0.0),
        (
            out.winner_id.clone(),
            Some("baseline".into()),
            winner.internal_gain,
        ),
    ];
    for p in &out.recursion {
    ...
    write_file(
        &viz.join("perspective_tree.svg"),
        &tree_svg(
            &tree,
            "perspective lineage — each child accepted only on internal raw-silhouette gain",
        ),
    );
    // held-out bars
    ...
    write_file(
        &viz.join("heldout_accuracy.svg"),
        &bars_svg(
            &acc_entries,
            "held-out latent-role 1-NN accuracy (95% bootstrap CI)",
            "frozen transforms applied to test embeddings; red line = CI",
        ),
    );
    ...
    write_file(
        &viz.join("heldout_link_auc.svg"),
        &bars_svg(
            &auc_entries,
            "held-out same-flow link AUC (cross-domain test pairs)",
            "does the perspective expose cross-cutting relations raw cosine cannot?",
        ),
    );
