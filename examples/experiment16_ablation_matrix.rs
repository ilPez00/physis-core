...
fn run(representation: &str, embedder: &impl VectorEmbed) -> Vec<ConfigResult> {
    ...
    let pure_emb: Vec<Vec<f32>> = PURE
        .iter()
        .map(|(_, bare, ctx, ..)| embedder.embed(if representation == "bare" { bare } else { ctx }))
        .collect();
    ...
    let true_coarses = |i: usize| -> Vec<&'static str> {
        if i < n_pure {
            ...
            vec![CROSS[i - n_pure].3, CROSS[i - n_pure].4]
        }
    };
    let true_fines = |i: usize| -> Vec<&'static str> {
        if i < n_pure {
            ...
            FINE_CATS
                .iter()
                .filter(|c| fine_to_coarse(c) == f1 || fine_to_coarse(c) == f2)
                .copied()
                .collect()
        }
    };
    ...
    let coarse_centroids: HashMap<&'static str, Vec<f32>> = COARSE_CATS
        .iter()
        .map(|&c| {
    ...
    let coarse_sims_for = |e: &Vec<f32>| -> HashMap<&'static str, f32> {
        COARSE_CATS
            .iter()
            .map(|&c| (c, cosine_sim(e, &coarse_centroids[c])))
            .collect()
    };
    let all_coarse_sims: Vec<HashMap<&'static str, f32>> =
        all_emb.iter().map(coarse_sims_for).collect();
    let real_coarse_argmax: Vec<&'static str> = all_coarse_sims
        .iter()
        .map(|s| {
            *s.iter()
        ...
        .collect();
    let coarse_correct: Vec<bool> = (0..n)
        .map(|i| true_coarses(i).contains(&real_coarse_argmax[i]))
        .collect();

    // ── Coarse-level multi-membership calibration (single delta, only 3 classes so no split needed) ──
    let coarse_membership_set =
        |sims: &HashMap<&'static str, f32>, delta: f32| -> Vec<&'static str> {
    ...
    let coarse_fpr_at = |delta: f32| -> f32 {
        (0..n_pure)
            .filter(|&i| coarse_membership_set(&all_coarse_sims[i], delta).len() > 1)
            .count() as f32
            / n_pure as f32
    };
    ...
    let coarse_sweep: Vec<(f32, f32, usize)> = (0..=30)
        .map(|i| {
            let d = i as f32 * 0.01;
            (d, coarse_fpr_at(d), coarse_recall_at(d))
        })
        .collect();
    let max_recall = coarse_sweep.iter().map(|(_, _, r)| *r).max().unwrap();
    let (coarse_delta, coarse_fpr, coarse_recall) = *coarse_sweep
        .iter()
    ...
    let all_fine_sims: Vec<HashMap<&'static str, f32>> =
        all_emb.iter().map(fine_sims_for).collect();
    let sibling_fpr_at = |delta: f32| -> f32 {
        (0..n_pure)
            .filter(|&i| {
                let own = PURE[i].4;
                all_fine_sims[i][own] - all_fine_sims[i][sibling_of(own)] < delta
            })
            .count() as f32
            / n_pure as f32
    };
    let sibling_delta = (0..=30)
        .map(|i| i as f32 * 0.01)
        .find(|&d| sibling_fpr_at(d) == 0.0)
        .unwrap_or(0.30);
    let cross_branch_fpr_at = |delta: f32| -> f32 {
        (0..n_pure)
            .filter(|&i| {
                ...
                let other_max = FINE_CATS
                    .iter()
                    .filter(|c| fine_to_coarse(c) != fine_to_coarse(own))
                    .map(|c| all_fine_sims[i][*c])
    ...
    let cross_branch_recall_at = |delta: f32| -> usize {
        (n_pure..n)
            .filter(|&i| {
                let (_, _, _, f1, f2, _) = CROSS[i - n_pure];
                let (&top_cat, &top_val) = all_fine_sims[i]
                    .iter()
                    .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
                    .unwrap();
                ...
                FINE_CATS
                    ...
                    .any(|c| all_fine_sims[i][*c] >= top_val - delta)
            })
            .count()
    };
    let cb_sweep: Vec<(f32, f32, usize)> = (0..=30)
        .map(|i| {
            let d = i as f32 * 0.01;
    ...
    let (cross_branch_delta, ..) = *cb_sweep
        .iter()
        .find(|(_, _, r)| *r == cb_max_recall)
        .unwrap();
    ...
    let fine_membership_set = |i: usize| -> Vec<&'static str> {
        ...
        for &c in &FINE_CATS {
            ...
            let is_sibling_of_top = FINE_CATS
                .iter()
                .any(|t| sims[*t] == top && sibling_of(t) == c);
            ...
            if is_top || within_sibling || within_cross_branch {
                ms.push(c);
            }
        }
        ms
    ...
    struct Branch {
        ...
        split: Vec<usize>,
        balance: f32,
        leaf_labels: [String; 2],
    }
    let mut branches = Vec::new();
    for &coarse in &COARSE_CATS {
        let members: Vec<usize> = (0..n)
            .filter(|&i| real_coarse_argmax[i] == coarse)
            .collect();
        if members.len() < 2 {
            continue;
        }
        ...
        for (c, slot) in leaf_labels.iter_mut().enumerate() {
            ...
            for (li, &gi) in members.iter().enumerate() {
                if split[li] == c && gi < n_pure {
                    *counts.entry(PURE[gi].4).or_insert(0) += 1;
                }
            }
            if let Some((label, _)) = counts.into_iter().max_by_key(|(_, c)| *c) {
    ...
    results.push(ConfigResult {
        name: "1: flat (coarse-only, single-label)",
        final_accuracy: acc1,
        coarse_mm_recall: None,
        coarse_mm_fpr: None,
        fine_recursed_branches: 0,
    });
    ...
    results.push(ConfigResult {
        name: "2: flat + multi-membership (coarse)",
        final_accuracy: acc1,
        coarse_mm_recall: Some((coarse_recall, n_cross)),
        coarse_mm_fpr: Some(coarse_fpr),
        fine_recursed_branches: 0,
    });
    ...
    results.push(ConfigResult {
        name: "3: + blind recursion (no gate, no MM)",
        final_accuracy: acc3,
        coarse_mm_recall: None,
        coarse_mm_fpr: None,
        fine_recursed_branches: fine_recursed_branches_blind,
    });

    // Config 4: + certified recursion (gate blocks bad branches -> those items scored at coarse level instead)
    let acc4 = (0..n)
        .filter(|&i| {
            if branch_certified(real_coarse_argmax[i]) {
                fine_correct_for_item(i)
            } else {
                coarse_correct[i]
            }
        })
        ...
        / n as f32;
    results.push(ConfigResult {
        name: "4: + certified recursion (gate, no MM)",
        final_accuracy: acc4,
        coarse_mm_recall: None,
        coarse_mm_fpr: None,
        fine_recursed_branches: fine_recursed_branches_certified,
    });

    // Config 5: multi-membership + blind recursion (fine correctness now also checks fine MM recall as a secondary stat)
    results.push(ConfigResult {
        name: "5: multi-membership + blind recursion",
        final_accuracy: acc3,
        coarse_mm_recall: Some((coarse_recall, n_cross)),
        coarse_mm_fpr: Some(coarse_fpr),
        fine_recursed_branches: fine_recursed_branches_blind,
    ...
    results.push(ConfigResult {
        name: "6: FULL (multi-membership + certified recursion)",
        final_accuracy: acc4,
        coarse_mm_recall: Some((coarse_recall, n_cross)),
        coarse_mm_fpr: Some(coarse_fpr),
        fine_recursed_branches: fine_recursed_branches_certified,
    ...
    let fine_mm_recall = (n_pure..n)
        ...
        .count();
    let fine_mm_fpr = (0..n_pure)
        .filter(|&i| fine_membership_set(i).len() > 1)
        .count() as f32
        / n_pure as f32;
    ...
    println!(
        "{:<50} {:>10} {:>16} {:>10}",
        "config", "final-acc", "coarse-MM r/fpr", "#branches recursed"
    );
    for r in &results {
        ...
        println!(
            "{:<50} {:>10.3} {:>16} {:>10}",
            r.name, r.final_accuracy, mm, r.fine_recursed_branches
        );
    }
    ...
    println!(
        "real coarse argmax assignment: {}/{} items correct (best-case credit for cross items)",
        (0..n).filter(|&i| coarse_correct[i]).count(),
        n
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        for (label, results) in [("bare", &bare_results), ("contextual", &ctx_results)] {
            ...
            println!(
                ...
                full - flat,
                if full > flat {
                    "FULL system beats the flat baseline"
                } else if full == flat {
                    "FULL system performs EXACTLY the same as the flat baseline — the added machinery earned nothing on final accuracy here"
                } else {
                    "FULL system performs WORSE than the flat baseline — recursion+certification actively hurt on this run"
                }
            );
        }
