...
fn main() {
    println!(
        "Experiment 24: cross-embedder agreement as a certification signal for CLUSTERING splits\n"
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        struct Case {
            name: &'static str,
            correct: bool,
            _minilm_emb: Vec<Vec<f32>>,
            bge_emb: Vec<Vec<f32>>,
            given_partition: Vec<usize>,
        }
        ...
        let econ_minilm: Vec<Vec<f32>> = ECONOMIC_BRANCH
            .iter()
        ...
        let mech_minilm: Vec<Vec<f32>> = MECHANICAL_BRANCH
            .iter()
            .map(|(_, s)| minilm.embed(s))
            .collect();
        let mech_bge: Vec<Vec<f32>> = MECHANICAL_BRANCH
            .iter()
            .map(|(_, s)| bge.embed(s))
            .collect();
        ...
        let cases = [
            Case {
                name: "1: GOOD-balanced (Dataset A true 8v7)",
                correct: true,
                _minilm_emb: a_minilm,
                bge_emb: a_bge,
                given_partition: a_true,
            },
            Case {
                name: "2: BAD-imbalanced (Dataset A real, 13v2)",
                correct: false,
                _minilm_emb: DATASET_A.iter().map(|(_, s, _)| minilm.embed(s)).collect(),
                bge_emb: DATASET_A.iter().map(|(_, s, _)| bge.embed(s)).collect(),
                given_partition: a_real,
            },
            Case {
                name: "3: BAD-balanced (Iter.10 bird branch, 6v4)",
                correct: false,
                _minilm_emb: bird_minilm,
                bge_emb: bird_bge,
                given_partition: bird_real,
            },
            Case {
                name: "4: GOOD-imbalanced (true wheel count, 2v10)",
                correct: true,
                _minilm_emb: veh_minilm,
                bge_emb: veh_bge,
                given_partition: veh_true,
            },
            Case {
                name: "5: BAD-balanced (Iter.12 economic branch)",
                correct: false,
                _minilm_emb: econ_minilm,
                bge_emb: econ_bge,
                given_partition: econ_real,
            },
            Case {
                name: "6: BAD-imbalanced (Iter.12 mechanical branch)",
                correct: false,
                _minilm_emb: mech_minilm,
                bge_emb: mech_bge,
                given_partition: mech_real,
            },
        ];

        let case_names: [&[(&str, &str)]; 6] = [&[], &[], &[], &[], &[], MECHANICAL_BRANCH];
        ...
        for (ci, c) in cases.iter().enumerate() {
            ...
            if ci == 0 || ci == 5 {
                let names: Vec<&str> = if ci == 0 {
                    DATASET_A.iter().map(|(n, ..)| *n).collect()
                } else {
                    case_names[5].iter().map(|(n, _)| *n).collect()
                };
        ...
        println!(
            "min(good)={min_good:.3}, max(bad)={max_bad:.3} -> separates: {}",
            min_good > max_bad
        );
