...
fn c4_return_interval(z: &[f64]) -> f64 {
    ...
    let g = if gaps.is_empty() {
        ...
        gaps.iter().map(|&x| x as f64).sum::<f64>() / gaps.len() as f64
    };
...
fn evaluate(
    ...
    let margin = if range.is_finite() && range > 0.0 {
        0.40 * range
    } else {
        f64::NAN
    };
    ...
    let spec = margin.is_finite()
        && disc_mean.is_finite()
        && nulls
            .iter()
            .all(|&nv| nv.is_finite() && (disc_mean - nv).abs() > margin);

    // held-out: both pre-frozen representations agree with the discovery value
    let ho = margin.is_finite()
        && disc_mean.is_finite()
        && holdout[0].is_finite()
        && holdout[1].is_finite()
    ...
    CandidateReport {
        ...
        discovery_value: disc_mean,
        discovery_spread: disc_spread,
        null_values: nulls,
        held_out_values: holdout.to_vec(),
        transfers,
        arithmetic_specific: spec,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lam_prefix: Vec<f64> = (0..=n_max)
        .scan(0.0f64, |a, k| {
            *a += base.lambda[k];
            Some(*a)
        })
    ...
    let theta: Vec<f64> = sample_prefix(
        &|x: usize| base.theta_prefix[x] - x as f64,
        n_max,
        n_samples,
    );
    let pi: Vec<f64> = sample_prefix(
        &|x: usize| {
    ...
    let mertens: Vec<f64> = sample_prefix(
        &|x: usize| base.mertens_prefix[x] / (x as f64).sqrt(),
        n_max,
        n_samples,
    );
    ...
    for ci in 0..5 {
        let sv: Vec<Vec<f64>> = [psi.clone(), theta.clone(), pi.clone(), mertens.clone()]
            ...
            .map(|s| zscore(&s))
            .collect();
        let disc: Vec<f64> = [0usize, 1]
            .iter()
            .map(|&k| all_candidates(&sv[k])[ci])
            .collect();
        let hold: Vec<f64> = [2usize, 3]
            .iter()
            .map(|&k| all_candidates(&sv[k])[ci])
            .collect();
    ...
    let verdict = if n_specific_survivors > 0 {
        ...
        if !any_transfer {
            reasons.push("no candidate transfers across independent representations");
        }
        if !any_specific {
            reasons.push("no candidate is arithmetic-specific against all nulls (the shared probability structure is generic, not prime-specific)");
        }
        if !any_ho {
            reasons.push("no candidate survives the held-out representations");
        }
    ...
    let representatives: Vec<RepresentationStat> = reps
        .into_iter()
        .map(|(n, k, v)| {
            let z = zscore(&v);
