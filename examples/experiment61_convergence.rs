//! EXPERIMENT 61: CONVERGENCE TEST — does the E60 G3 miss close at N=1e7?
//!
//! E60 (N=1e6) PARTIAL: G1+G2 pass, G3 miss (fit 1.47/1.49 vs 1.32032±0.15).
//! E61 runs E60's exact estimator at N=1e7. If the fitted 2C2 moves toward
//! 1.32032 the miss is a finite-N artifact; if it stays ~1.47 the estimator
//! is biased. Only change from E60: n_max 1e6 -> 1e7.
//! Frozen spec: research/impossible_machine/EXPERIMENT61_SPEC.md.
...
struct E61Output {
    ...
    e60_fit_disc: f64, e60_fit_hold: f64,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== EXPERIMENT 61: CONVERGENCE TEST (N=1e7) ===");
    let n_max = 10_000_000usize;
    let kmax = 40usize;
    ...
    let sv = g1 && g2 && g3;
    let (e60d, e60h) = (1.4701501691364305f64, 1.4865567864996678f64);
    let dt_d = (fd - C2X2).abs();
    let dt_e60 = (e60d - C2X2).abs();
    let delta_to_target = dt_d - dt_e60; // +ve = further from 1.32032 than E60
    let verdict = if sv {
        format!("AGREEMENT — 2C2 disc={:.3} hold={:.3} in {:.5}±{:.2} at N=1e7 (E60 {:.3}/{:.3}); G1 D={:.3} vs nulls {:.3}; G2 stable.",
            fd, fh, C2X2, G3H, e60d, e60h, dd, mx)
    } else {
        let mut r = Vec::new();
        if !g1 { r.push("G1 fail".to_string()); }
        if !g2 { r.push("G2 fail".to_string()); }
        if !g3 { r.push(format!("G3 miss: {:.3}/{:.3} vs {:.5}±{:.2} (E60 {:.3}/{:.3}, Δ-to-target {:+.3})",
            fd, fh, C2X2, G3H, e60d, e60h, delta_to_target)); }
        if delta_to_target <= -0.02 {
            format!("PARTIAL — {}. Converging toward 1.32032; needs N=1e8.", r.join("; "))
        } else if delta_to_target <= 0.0 {
            format!("PARTIAL — {}. Flat between N=1e6 and N=1e7; estimator not yet N-converged.", r.join("; "))
        } else if delta_to_target <= 0.05 {
            format!("PARTIAL — {}. DIVERGING from 1.32032 with N; estimator biased high, not a finite-N artifact.", r.join("; "))
        } else {
            format!("MISS — {}. Strong divergence; this estimator cannot recover 2C2.", r.join("; "))
        }
    };
    let rel = format!("no 1/2 in gen/score; fit {:.3}/{:.3} at N=1e7 (E60 {:.3}/{:.3}); Δ-to-target {:+.3}; D {:.3}/{:.3}",
        fd, fh, e60d, e60h, delta_to_target, dd, dh);
    let out = E61Output {
        title: "Singular-series fit convergence (2C2) — N=1e6 vs N=1e7".into(),
        context: "E60 PARTIAL at N=1e6 (fit 1.470/1.487 vs 1.32032±0.15). E61 same estimator at N=1e7: is the miss a finite-N artifact or estimator bias?".into(),
        n_max, kmax, seed: 20260912, e60_fit_disc: e60d, e60_fit_hold: e60h,
        ...
        artifacts: vec![
            "physis-core/examples/experiment61_convergence.rs".into(),
            "physis-core/research/impossible_machine/experiments/experiment61_results.json".into(),
            "physis-core/research/impossible_machine/EXPERIMENT61_RESULTS.md".into(),
        ],
    };
    let op = Path::new("research/impossible_machine/experiments/experiment61_results.json");
