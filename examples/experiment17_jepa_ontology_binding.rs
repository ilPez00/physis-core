...
impl Predictor {
    ...
    fn train_step_nway(
        ...
        for i in 0..n {
            let (pred_raw, h) = self.forward(&xs[i]);
            let logits: Vec<f32> = centroids
                .iter()
                .map(|c| pred_raw.iter().zip(c).map(|(a, b)| a * b).sum::<f32>() / tau)
                .collect();
            ...
            for c in 0..n_classes {
                ...
                for d in 0..self.dim {
                    grad_pred_raw[d] += coef * centroids[c][d];
                }
            }
            let mut grad_h = vec![0.0f32; self.k];
            for d in 0..self.dim {
...
fn run_real_domain_case(embedder: &impl VectorEmbed) {
    ...
    println!(
        "Motivation: the 6 toy cases above have only 2-5 held-out items each — far too coarse"
    );
    ...
    let train_centroids: Vec<Vec<f32>> = (0..n_classes)
        .map(|c| {
    ...
    let raw_hits = holdout_idx
        .iter()
        .filter(|&&i| nway_argmax_correct(&embeddings[i], &train_centroids, labels[i]))
        .count();
    println!(
        "raw-embedding held-out accuracy (baseline, no predictor): {:.3}",
        raw_hits as f32 / holdout_idx.len() as f32
    );
    ...
    for epoch in 0..800 {
        final_loss =
            predictor.train_step_nway(&train_xs, &train_labels, &train_centroids, 0.1, 0.3);
        if epoch % 100 == 0 || epoch == 799 {
            println!("  epoch {epoch}: loss={final_loss:.4}");
        }
    }
    ...
    let mean_pairwise = |vecs: &[Vec<f32>]| -> f32 {
        ...
        for a in 0..n {
            for b in (a + 1)..n {
                total += cosine_sim(&vecs[a], &vecs[b]);
                count += 1;
            }
        }
        if count == 0 {
    ...
    let train_hits = train_idx
        .iter()
        .filter(|&&i| nway_argmax_correct(&all_pred[i], &train_centroids, labels[i]))
        .count();
    let trained_holdout_hits = holdout_idx
        .iter()
        .filter(|&&i| nway_argmax_correct(&all_pred[i], &train_centroids, labels[i]))
        .count();
    println!(
...
struct CaseResult {
    ...
    correct: bool,
    raw_holdout_acc: f32,
    trained_holdout_acc: f32,
    final_loss: f32,
    gen_gap: f32,
    output_norm: f32,
    pairwise_cos: f32,
    raw_pairwise_cos: f32,
}
...
fn run_case(
    ...
    let raw_holdout_hits = holdout_idx
        .iter()
        .filter(|&&i| {
    ...
    CaseResult {
        ...
        trained_holdout_acc,
        final_loss,
        gen_gap: train_acc - trained_holdout_acc,
        output_norm,
        pairwise_cos,
        raw_pairwise_cos,
    }
...
fn main() {
    println!(
        "Experiment 17: genuine JEPA-style ontology-binding predictor as a certification signal\n"
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        let results = vec![
            ...
            run_case(
                "2: BAD-imbalanced (Dataset A real, 13v2)",
                false,
                DATASET_A
                    .iter()
                    .map(|(_, s, _)| embedder.embed(s))
            ...
            run_case(
                "3: BAD-balanced (Iter.10 bird branch, 6v4)",
                false,
                bird_emb,
                bird_real,
            ),
            run_case(
                "4: GOOD-imbalanced (true wheel count, 2v10)",
                true,
                veh_emb,
                veh_true,
            ),
            run_case(
                "5: BAD-balanced (Iter.12 economic branch)",
                false,
                econ_emb,
                econ_real,
            ),
            run_case(
                "6: BAD-imbalanced (Iter.12 mechanical branch)",
                false,
                mech_emb,
                mech_real,
            ),
        ...
        println!(
            "{:<48} {:>8} {:>10} {:>10} {:>9} {:>10} {:>10} {:>10}",
            "case", "raw-acc", "train-acc", "held-acc", "gen-gap", "loss", "out-norm", "pair-cos"
        );
        for r in &results {
            println!(
            ...
            println!(
                "    raw pairwise cosine (for collapse comparison): {:.3}",
                r.raw_pairwise_cos
            );
        }
        ...
        let raw_scores: Vec<(bool, f32)> = results
            .iter()
            .map(|r| (r.correct, r.raw_holdout_acc))
            .collect();
        let trained_scores: Vec<(bool, f32)> = results
            .iter()
            .map(|r| (r.correct, r.trained_holdout_acc))
            .collect();
        let loss_scores: Vec<(bool, f32)> =
            results.iter().map(|r| (r.correct, -r.final_loss)).collect(); // lower loss = better, invert
        let gap_scores: Vec<(bool, f32)> =
            results.iter().map(|r| (r.correct, -r.gen_gap)).collect(); // smaller gap = better, invert

        println!("\n=== Verdict: does min(correct-case scores) > max(wrong-case scores)? ===");
        println!(
            "raw-embedding held-out accuracy (baseline, no predictor): {}",
            separates(&raw_scores)
        );
        println!(
            "trained-predictor held-out accuracy:                      {}",
            separates(&trained_scores)
        );
        println!(
            "final training loss (lower=better, inverted):             {}",
            separates(&loss_scores)
        );
        println!(
            "generalization gap (smaller=better, inverted):            {}",
            separates(&gap_scores)
        );
