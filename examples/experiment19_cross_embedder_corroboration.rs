...
fn local_split_membership_sets(all_emb: &[Vec<f32>], n_pure: usize) -> Vec<Vec<&'static str>> {
    ...
    let sibling_fpr_at = |delta: f32| -> f32 {
        (0..n_pure)
            .filter(|&i| {
                let own = PURE[i].4;
                all_sims[i][own] - all_sims[i][sibling_of(own)] < delta
            })
            .count() as f32
            / n_pure as f32
    };
    ...
    let cross_branch_recall_at = |delta: f32| -> usize {
        (n_pure..all_emb.len())
            .filter(|&i| {
                ...
                let (&top_cat, &top_val) = all_sims[i]
                    .iter()
                ...
                FINE_CATS
                    ...
                    .any(|c| all_sims[i][*c] >= top_val - delta)
            })
            .count()
    };
    let cb_sweep: Vec<(f32, usize)> = (0..=30)
        .map(|i| {
            let d = i as f32 * 0.01;
            (d, cross_branch_recall_at(d))
        })
        .collect();
    let cb_max_recall = cb_sweep.iter().map(|(_, r)| *r).max().unwrap();
    let cross_branch_delta = cb_sweep
        .iter()
        .find(|(_, r)| *r == cb_max_recall)
        .unwrap()
        .0;
    ...
    (0..all_emb.len())
        .map(|i| {
            ...
            for &c in &FINE_CATS {
                ...
                let within_cross_branch =
                    !is_sibling_of_top && (top - sims[c] < cross_branch_delta);
...
fn run_vehicle_test(minilm: &OnnxEmbedder, bge: &OnnxEmbedder, representation: &str) {
    ...
    let baseline_recall = (n_pure..n)
        .filter(|&i| {
            let (_, _, _, f1, f2, _) = CROSS[i - n_pure];
            let coarses: std::collections::HashSet<&str> =
                minilm_sets[i].iter().map(|c| fine_to_coarse(c)).collect();
    ...
    let corroborated_sets: Vec<Vec<&str>> = (0..n)
        .map(|i| {
            let own_top = {
                // recompute own top fine cat for this item under MiniLM for reference labeling
                minilm_sets[i][0]
            };
            minilm_sets[i]
                .iter()
                .filter(|&&c| c == own_top || bge_sets[i].contains(&c))
                .copied()
                .collect()
        })
        .collect();
    let corroborated_fp = (0..n_pure)
        .filter(|&i| corroborated_sets[i].len() > 1)
        .count();
    let corroborated_recall = (n_pure..n)
        .filter(|&i| {
            let (_, _, _, f1, f2, _) = CROSS[i - n_pure];
            let coarses: std::collections::HashSet<&str> = corroborated_sets[i]
                .iter()
                .map(|c| fine_to_coarse(c))
                .collect();
            coarses.contains(f1) && coarses.contains(f2)
        })
        .count();
    ...
    println!("\n--- {representation} ---");
    println!(
        "MiniLM alone (Iteration 13 local/split calibration): FPR={:.3} ({}/{}), recall={}/{}",
        baseline_fp as f32 / n_pure as f32,
        baseline_fp,
        n_pure,
        baseline_recall,
        n_cross
    ...
    println!(
        "+ BGE corroboration filter:                          FPR={:.3} ({}/{}), recall={}/{}",
        corroborated_fp as f32 / n_pure as f32,
        corroborated_fp,
        n_pure,
        corroborated_recall,
        n_cross
    ...
    if baseline_fp > 0 {
        ...
        for i in 0..n_pure {
            if corroborated_sets[i].len() > 1 {
                println!(
                    "  {:<20} MiniLM-alone={:?}  after-corroboration={:?}",
                    PURE[i].0, minilm_sets[i], corroborated_sets[i]
                );
            }
        }
    }
...
fn run_real_ontology_test(minilm: &OnnxEmbedder, bge: &OnnxEmbedder) {
    ...
    let sims_for = |e: &Vec<f32>, centroids: &[Vec<f32>]| -> Vec<f32> {
        centroids.iter().map(|c| cosine_sim(e, c)).collect()
    };
    let minilm_sims: Vec<Vec<f32>> = minilm_emb
        .iter()
        .map(|e| sims_for(e, &minilm_centroids))
        .collect();
    let bge_sims: Vec<Vec<f32>> = bge_emb
        .iter()
        .map(|e| sims_for(e, &bge_centroids))
        .collect();
    ...
    for delta in [0.02, 0.03, 0.05] {
        let minilm_flagged: Vec<usize> = (0..n)
            .filter(|&i| candidate_set(&minilm_sims[i], delta).len() > 1)
            .collect();
        let corroborated_count = minilm_flagged
            .iter()
            .filter(|&&i| {
    ...
    for i in 0..n {
        ...
        let own_top = labels[i];
        let bge_set = candidate_set(&bge_sims[i], delta);
        let corroborated: Vec<&str> = m_set
            .iter()
            .filter(|&&c| c != own_top && bge_set.contains(&c))
            .map(|&c| domain_names[c].as_str())
            .collect();
        println!(
...
fn main() {
    println!(
        "Experiment 19: cross-embedder corroboration for multi-membership (roadmap item 15)\n"
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        let (minilm, bge) = match (minilm, bge) {
            ...
            _ => {
                println!("WARNING: MiniLM or BGE not available — aborting.");
                return;
            }
        };
