...
fn main() {
    println!(
        "Experiment 10: closing gaps 1-3 — calibration, end-to-end integration, DAG protection\n"
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        println!(
            "{:>6}  {:>18}  {:>10}  {:>14}",
            "delta", "FPR on 15 pure", "recall/4", "avg |membership|"
        );
        ...
        for &delta in &deltas {
            ...
            for (idx, s) in pure_sims.iter().enumerate() {
                total_membership += membership_set(s, delta).len();
                let _ = idx;
            }
            for (idx, s) in cross_sims.iter().enumerate() {
                ...
                if ms.contains(&f1) && ms.contains(&f2) {
                    recall_hits += 1;
                }
            }
            let avg = total_membership as f32 / (n_pure + CROSS.len()) as f32;
            println!(
                "{delta:>6.2}  {fpr:>18.3}  {:>7}/{}    {avg:>14.2}",
                recall_hits,
                CROSS.len(),
                avg = avg
            );
        ...
        let delta = match chosen_delta {
            ...
            None => {
                ...
                let best = deltas
                    ...
                    .max_by(|&a, &b| {
                        let ra = cross_sims
                            ...
                            .enumerate()
                            .filter(|(i, s)| {
                                let ms = membership_set(s, a);
                                let (_, _, f1, f2, _) = CROSS[*i];
                                ms.contains(&f1) && ms.contains(&f2)
                            })
                            .count();
                        let rb = cross_sims
                            .iter()
                ...
                let fp = pure_sims
                    .iter()
                    .filter(|s| membership_set(s, best).len() > 1)
                    .count();
        ...
        println!(
            "REAL (discovered) depth-1 split sizes: {} / {}",
            real_depth1.iter().filter(|&&a| a == 0).count(),
            real_depth1.iter().filter(|&&a| a == 1).count()
        );
        ...
        let oracle_depth1: Vec<usize> = (0..all_emb.len())
            .map(|i| if coarse_of(i) == "mammal" { 0 } else { 1 })
            .collect();
        ...
        for coarse_branch in ["mammal", "bird"] {
            let members: Vec<usize> = (0..all_emb.len())
                .filter(|&i| coarse_of(i) == coarse_branch)
                .collect();
            ...
            for c in 0..2 {
                ...
                for &gi in &leaf_members {
                    if gi < n_pure {
                        *counts.entry(PURE[gi].3).or_insert(0) += 1;
                    }
                }
                let label = counts
                    .into_iter()
                ...
                leaves.push(Leaf {
                    label: format!("{coarse_branch}.{label}"),
            ...
            for &gi in &leaf.members {
                ...
                for &cat in &ms {
                        ...
                        .position(|l| l.label.ends_with(&format!(".{cat}")))
                    {
                        ...
                        let cross_branch = leaves[target].label.split('.').next().unwrap()
                            != leaf.label.split('.').next().unwrap();
                        println!(
                            "  {name:<14} home='{}' -> also linked to '{}'{}",
                            leaf.label,
                            leaves[target].label,
                            if cross_branch {
                                "  [CROSS-BRANCH — this is the DAG case]"
                            } else {
                                ""
                            }
                        );
        ...
        if cross_links.is_empty() {
            println!(
                "  (none at this delta — calibration in gap 1 chose a conservative tolerance)"
            );
        }
