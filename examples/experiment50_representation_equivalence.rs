...
fn finite(v: &[f64]) -> Vec<f64> {
    v.iter().cloned().filter(|x| x.is_finite()).collect()
}
...
fn candidates(a: &Representation, b: &Representation, shuffle: &Representation) -> Candidates {
    ...
    for (wa, wb) in a.windows.iter().zip(b.windows.iter()) {
        let va: Vec<f64> = finite(wa);
        let vb: Vec<f64> = finite(wb);
        if va.len() >= 3 && vb.len() >= 3 && va.len() == vb.len() {
            c5s.push(kendall(&va, &vb));
        }
    }
    let c5 = if c5s.is_empty() {
        f64::NAN
    } else {
        c5s.iter().sum::<f64>() / c5s.len() as f64
    };
    Candidates {
        c1,
        c2_diff,
...
struct CandReport {
    ...
    complexity: usize,
    discovery_value: f64,
    discovery_null_ref: f64,
    survives_discovery: bool,
    heldout_values: Vec<(String, f64)>,
    heldout_survives: Option<bool>,
    specificity_prime: f64,
    specificity_shuffle: f64,
    specificity_cramer: f64,
    arithmetic_specific: Option<bool>,
}
...
fn main() {
    ...
    for (name, y, exp) in &control_specs {
        ...
        for k in 0..4 {
            if exp[k].is_finite() {
                if (est[k] - exp[k]).abs() < 0.3 {
                    hits += 1;
                }
                if (est[k] - exp[k]).abs() > 0.6 {
                    bad += 1;
                }
            }
        }
        ...
        println!(
            "  control {}: {:?} -> {}",
            name,
            est.iter().map(|v| v.round() as i64).collect::<Vec<_>>(),
    ...
    println!(
        "  E49 reproduction: E_psi spread {:.3} vs delta {:.3} -> {}",
        sp_psi,
        dl_psi,
        if reproduced { "OK" } else { "REFUSED" }
    );
    ...
    let mk = |name: &str, ys: &[f64]| -> Representation {
        Representation {
            name: name.into(),
            ys: ys.to_vec(),
            windows: window_profile(ys, n_win),
        }
    };
    ...
    if !reproduced {
        let out = Output50 {
            e50_status: "REFUSED",
            reproduced_e49: false,
            representations_tested: vec![],
            nulls_tested: vec![],
            candidates: vec![],
            representation_equivalence: "not evaluated".into(),
            dimension_invariance: "not evaluated".into(),
            arithmetic_specificity: "not evaluated".into(),
            relation_to_half: "not evaluated".into(),
        ...
        write_file(
            std::path::Path::new(
                "research/impossible_machine/experiments/experiment50_results.json",
            ),
    ...
    let surv = |v: f64, nv: f64| -> bool {
        if v.is_nan() || nv.is_nan() {
            false
        } else {
            (v - nv).abs() > 0.15
        }
    ...
    let s2 = disc.c2_diff.is_finite() && disc.c2_diff < 0.15 && nul1.c2_diff >= disc.c2_diff
        || (disc.c2_diff.is_finite() && nul2.c2_diff.is_finite() && disc.c2_diff < nul2.c2_diff);
    ...
    println!(
        "    C1 rank-corr d-profiles:      {:.3} (null {:.3}) -> {}",
        disc.c1,
        nul1.c1.min(nul2.c1),
        if s1 { "SURVIVES" } else { "fails" }
    );
    println!(
        "    C2 drift-diff:                {:.3} (null {:.3}/{:.3}) -> {}",
        disc.c2_diff,
        nul1.c2_diff,
        nul2.c2_diff,
        if s2 { "SURVIVES" } else { "fails" }
    );
    println!(
        "    C3 null-ordering |diff|:      {:.3} (null {:.3}) -> {}",
        c3_disc,
        c3_null,
        if s3 { "SURVIVES" } else { "fails" }
    );
    println!(
        "    C4 raw-Hurst rank-corr:       {:.3} (null {:.3}) -> {}",
        disc.c4,
        nul1.c4.min(nul2.c4),
        if s4 { "SURVIVES" } else { "fails" }
    );
    println!(
        "    C5 estimator-order Kendall:   {:.3} (null {:.3}) -> {}",
        disc.c5,
        nul1.c5.max(nul2.c5),
        if s5 { "SURVIVES" } else { "fails" }
    );
    ...
    let heldout = |id: &str, disc_v: f64, vals: &[f64], high_is_good: bool| -> CandReport {
        let hv: Vec<(String, f64)> = vec![
            ("psi-pi".into(), vals[0]),
            ("theta-mertens".into(), vals[1]),
            ("pi-mertens".into(), vals[2]),
        ];
        let survives = if !vals.iter().all(|v| v.is_finite()) {
            None
        } else {
            Some(if high_is_good {
                vals.iter().all(|v| *v > disc_v - 0.15)
            } else {
                vals.iter().all(|v| *v < disc_v + 0.15)
            })
        };
        CandReport {
            id: id.into(),
            complexity: 0,
            discovery_value: disc_v,
            discovery_null_ref: f64::NAN,
            survives_discovery: true,
            heldout_values: hv,
            heldout_survives: survives,
            specificity_prime: f64::NAN,
            specificity_shuffle: f64::NAN,
            specificity_cramer: f64::NAN,
            arithmetic_specific: None,
        }
    };
    let mut reports = Vec::new();
    if s1 {
        reports.push(heldout(
            "C1_rank_profiles",
            disc.c1,
            &[ho1.c1, ho2.c1, ho3.c1],
            true,
        ));
    }
    if s2 {
        reports.push(heldout(
            "C2_drift_diff",
            disc.c2_diff,
            &[ho1.c2_diff, ho2.c2_diff, ho3.c2_diff],
            false,
        ));
    }
    if s3 {
    ...
    for r in reports.iter_mut() {
        ...
        r.specificity_prime = pv;
        r.specificity_shuffle = sv;
        r.specificity_cramer = cv;
        r.arithmetic_specific = if pv.is_nan() {
            None
        } else {
            Some((pv - sv).abs() > 0.1 || (pv - cv).abs() > 0.1)
        };
        r.complexity = match r.id.as_str() {
            "C2_drift_diff" | "C3_null_ordering" => 1,
            _ => 2,
        };
    }

    let n_surv_disc = s1 as usize + s2 as usize + s3 as usize + s4 as usize + s5 as usize;
    let n_surv_ho = reports
        .iter()
        .filter(|r| r.heldout_survives == Some(true))
        .count();
    let n_specific = reports
        .iter()
        .filter(|r| r.arithmetic_specific == Some(true))
        .count();
    ...
    let (status, failure_mode, next) = if n_surv_disc == 0 {
        ...
        .iter()
        .any(|r| r.heldout_survives == Some(true) && r.arithmetic_specific == Some(true))
    {
        (
            "PARTIAL",
            "",
            "formalize the surviving invariant; test on a larger sieve and finer sampling",
        )
    } else {
    ...
    let best = reports
        .iter()
        .filter(|r| r.heldout_survives == Some(true) && r.arithmetic_specific == Some(true))
        .min_by_key(|r| r.complexity)
        .map(|r| (r.id.clone(), r.discovery_value))
    ...
    let out = Output50 {
        e50_status: status,
        reproduced_e49: reproduced,
        representations_tested: vec![
            Rep50 {
                name: "E_psi".into(),
                kind: "discovery".into(),
                converged_d: Some(1.401),
                window_d: profile_scalar(&r_psi.windows),
            },
            Rep50 {
                name: "E_theta".into(),
                kind: "discovery".into(),
                converged_d: None,
                window_d: profile_scalar(&r_th.windows),
            },
            Rep50 {
                name: "E_pi".into(),
                kind: "held-out".into(),
                converged_d: None,
                window_d: profile_scalar(&r_pi.windows),
            },
            Rep50 {
                name: "Mertens".into(),
                kind: "held-out".into(),
                converged_d: None,
                window_d: profile_scalar(&r_me.windows),
            },
        ],
        nulls_tested: vec!["shuffled-E_psi".into(), "Cramer".into()],
        candidates: reports,
        representation_equivalence: if n_surv_ho > 0 {
            "partial: some low-complexity structure transfers".into()
        } else {
            "no evidence of equivalence beyond the discovery pair".into()
        },
        dimension_invariance: "NO — E49 conclusion stands: d is not an invariant of the object"
            .into(),
        arithmetic_specificity: if n_specific > 0 {
            format!("{n_specific} candidate(s) distinguish primes from nulls")
        } else {
            "no candidate is arithmetic-specific".into()
        },
    ...
    write_file(
        std::path::Path::new("research/impossible_machine/experiments/experiment50_results.json"),
        &serde_json::to_string_pretty(&out).unwrap(),
    );
