...
fn true_coarse(i: usize) -> &'static str {
    CORPUS[i].2
}
...
fn purity(assignment: &[usize]) -> f32 {
    ...
    for c in 0..2 {
        ...
        for (i, a) in assignment.iter().enumerate() {
            if *a == c {
                *counts.entry(true_coarse(i)).or_insert(0usize) += 1;
            }
        }
...
fn main() {
    println!(
        "Experiment 9: does a certification signal distinguish a good split from a bad one?\n"
    );
    ...
    #[cfg(feature = "embed-onnx")]
    {
        ...
        let good: Vec<usize> = (0..CORPUS.len())
            .map(|i| if true_coarse(i) == "mammal" { 0 } else { 1 })
            .collect();

        for (name, assignment) in [
            ("BAD (k-means depth-1, Iteration 7)", &bad),
            ("GOOD (true mammal/bird)", &good),
        ] {
            let sizes = (
                assignment.iter().filter(|&&a| a == 0).count(),
                assignment.iter().filter(|&&a| a == 1).count(),
            );
            println!("=== {name} ===");
            println!("  sizes: {:?}", sizes);
            println!("  purity vs ground truth: {:.3}", purity(assignment));
            println!(
                "  balance ratio (candidate certification signal 1): {:.3}",
                balance_ratio(assignment)
            );
            println!(
                "  silhouette-like score (candidate certification signal 2): {:.3}\n",
